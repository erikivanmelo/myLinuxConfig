...
def get_status():
    spotify_read = subprocess.check_output("%s/get.sh status" % dir_path, shell=True)
    spotify_status=spotify_read.decode('utf-8')
    return spotify_status
    #sys.stdout.write(spotify_status)

def get_artist():
    spotify_read = subprocess.check_output("%s/get.sh artist" % dir_path, shell=True)
    spotify_artist=spotify_read.decode('utf-8')
    return spotify_artist[:-1]
    #sys.stdout.write(spotify_artist)

def get_song():
    spotify_read = subprocess.check_output("%s/get.sh song" % dir_path, shell=True)
